...
class MasterServicer(mapreduce_pb2_grpc.MasterServicer):
    ...
    def start_mappers(self, num_mappers):
        logging.info(f"Starting {num_mappers} mapper instances.")
        for mapper_id in range(BASE_PORT_MAPPER, BASE_PORT_MAPPER + num_mappers):
    ...
    def start_reducers(self, num_reducers):
        logging.info(f"Starting {num_reducers} recducer instances.")
        for reducer_id in range(BASE_PORT_REDUCER, BASE_PORT_REDUCER + num_reducers):
    ...
    def run(self, args):
        self.start_mappers(args.num_mappers)
        self.start_reducers(args.num_reducers)
